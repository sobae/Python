...
INITIAL_BCC_BITS = 469841028
#INITIAL_SWC_BITS = 402734313
INITIAL_SWC_BITS = 469841028

INITIAL_FX = 0.18
#INITIAL_TIMESTAMP = 1503430225
INITIAL_TIMESTAMP = 1526629343

INITIAL_HASHRATE = 500    # In PH/s.
INITIAL_HEIGHT = 86327
...
def print_state():
    ...
    fp = open("miner_H.txt", 'a')
    t_str= (', '.join(['{:d}'.format(state.height),
                 '{:.8f}'.format(state.fx),
                 '{:d}'.format(block_time),
                 '{:d}'.format(state.timestamp),
                 '{:%Y-%m-%d %H:%M:%S}'.format(t),
...
def revenue_ratio(fx, BCC_target):
    ...
    return 1 / SWC_difficulty_ratio

def median_time_past(states):
    times = [state.timestamp for state in states]
    return sorted(times)[len(times) // 2]

def next_bits_h(msg, mtp_window, high_barrier, target_raise_frac,
    ...
    if (states[-1].height +1) % 480 == 0:
    
        # Long term block production time stabiliser
        spend_time = states[-1].timestamp - states[-481].timestamp

        if spend_time < 86400/4:
           spend_time = 86400/4
        if spend_time > 86400*4:
           spend_time = 86400*4

        target = int(target * spend_time/86400)

        if spend_time < 86400:
            msg.append("480 block time difficulty raise {}, {}" .format(spend_time, target_to_bits(target)))
   
        elif spend_time > 86400:
            msg.append("Difficulty drop {}, {}" .format(spend_time, target_to_bits(target)))
    else:
        msg.append("Difficulty held {}" .format(target_to_bits(target)))
...
def block_time(mean_time):
    ...
    #sample = (random.betavariate(1,100))
    lmbda = 1 / mean_time
...
def next_step(algo, scenario, fx_jump_factor):
    ...
    var_fraction = random.random()*(VARIABLE_PCT / 100)
    ...
    if mean_rev_ratio >= 3000 + GREEDY_PCT / 100:
        if greedy_frac != 0.0:
            msg.append("Greedy miners left")
        greedy_frac = 0.0
    elif mean_rev_ratio <= 3000 : # - GREEDY_PCT / 100:
        if greedy_frac != 1.0:
    ...
    mean_hashes = pow(2, 256) // (target +1)
    mean_time = mean_hashes / (hashrate * 1e10)
    time = math.ceil(block_time(mean_time) + 0.5)
...
def run_one_simul(algo, scenario, print_it):
    ...
    for n in range(-N, 0):
        state = State(INITIAL_HEIGHT + n, INITIAL_TIMESTAMP + n * IDEAL_BLOCK_TIME,
                      ...
                      INITIAL_FX, INITIAL_HASHRATE, 1.0, True, '')
        states.append(state)
...
def main():
    ...
    parser.add_argument('-a', '--algo', metavar='algo', type=str,
                        ...
                        default = 'h-1', help='algorithm choice')
